...
public class Tools {
    ...
    //输出结果至文件
    public static void writeResult(String fileName, Population pop) throws IOException {
        /* 写入Txt文件 */
        File writeName = new File(fileName); // 相对路径，如果没有则要建立一个新文件
        writeName.createNewFile();
        BufferedWriter out = new BufferedWriter(new FileWriter(writeName, true));
        Individual[] indvs = pop.getSortIndvs();
        for (Individual indv : indvs) {
            for (int item : indv.getIndvCode()) {
                out.write(item + "\t");
            }
            out.write("\n");
            out.write(indv.getFitness() + "\n");
        }
        out.write("Best individual:" + "\n");
        for (int item : pop.getBestIndv().getIndvCode()) {
            out.write(item + "\t");
        }
        out.write("\n");
        out.write(pop.getBestIndv().getFitness() + "\n");
